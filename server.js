...
function buildSegmentPrompt(duration) {
  return [
    ...
    "segments 是按时间顺序排列的数组，每项必须包含 id、startTime、endTime、title、summary、keywords。",
    ...
    '输出示例：{"summary":"...","segments":[{"id":"seg-001","startTime":0,"endTime":32,"title":"开场","summary":"...","keywords":["钩子","场景"]}]}'
  ].join("\n");
}
...
function normalizeSegment(segment, index, duration) {
  ...
  return {
    ...
    color: segment.color || segmentColors[index % segmentColors.length],
...
function buildMockSegments(duration) {
  ...
  const blueprints = [
    { title: "开场引入", summary: "交代视频主题、场景背景与接下来要看的重点。", keywords: ["开场", "主题", "钩子"] },
    { title: "核心过程", summary: "展示主要内容推进，给出关键步骤或事件展开。", keywords: ["过程", "步骤", "推进"] },
    { title: "重点信息", summary: "汇总中段最值得关注的信息、结论或变化。", keywords: ["重点", "信息", "变化"] },
    { title: "收束总结", summary: "对前文进行收束，形成整体结论与下一步提示。", keywords: ["总结", "结论", "行动"] }
  ];
